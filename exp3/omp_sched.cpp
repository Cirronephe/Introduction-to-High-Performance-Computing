...
void prepareRandomParts(int nParts, int *parts) {
    ...
    std::mt19937 gen{0};
    std::normal_distribution<> d{(double)n / nParts, (double)n / nParts / 4};

    double totLength = 0;
    std::vector<double> lengths;
    lengths.reserve(nParts);
    ...
    parts[0] = 0;
    for (int i = 1; i < nParts; i++) {
        parts[i] = std::min(n, std::max(0, parts[i - 1] + (int)round(lengths[i] / totLength * n)));
    }
    parts[nParts] = n;
...
void sortEveryUniformParts(int nParts, int *parts) {
    ...
    #pragma omp parallel for num_threads(28) schedule(guided)
    for (int i = 0; i < nParts; i++) {
