...
void SpMMOpt::write_metis_graph(const std::string& filename) {
    ...
    std::vector<std::set<int>> adj(num_v);
    ...
    for (int u = 0; u < num_v; ++u) {
        for (int eid = h_ptr[u]; eid < h_ptr[u + 1]; ++eid) {
            ...
            int a = std::min(u, v);
            int b = std::max(u, v);
            if (undirected_edges.emplace(a, b).second) {
                adj[a].insert(b);
                adj[b].insert(a);
            }
        }
    ...
    ofs << num_v << " " << undirected_edges.size() << "\n";
    for (int u = 0; u < num_v; ++u) {
        bool first = true;
        for (int v : adj[u]) {
            if (!first) ofs << " ";
...
void SpMMOpt::preprocess(float *vin, float *vout) {
    write_metis_graph("/home/course/hpc/users/2023010828/PA3/graph/arxiv.graph");
    // return;
